    ...
    public ResponseEntity<ResponseMessage> uploadFile(@RequestParam("file") MultipartFile file) {
        ...
        if (CSVHandler.hasCSVFormat(file)) {
            try {
                ...
                return ResponseEntity.status(HttpStatus.OK).body(new ResponseMessage(feedBackMessage));
            } catch (Exception e) {
                log.error("Could not upload the file: {}", e.getMessage(), e);
                feedBackMessage = "Could not upload the file: " + file.getOriginalFilename() + "!";
                return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(new ResponseMessage(feedBackMessage));
            }
        }
        log.info("Please upload a csv file!: ");
        feedBackMessage = "Please upload a csv file!";
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResponseMessage(feedBackMessage));
    }
}
