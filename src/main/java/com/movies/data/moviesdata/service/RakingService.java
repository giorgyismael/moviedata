...
public class RakingService {
    ...
    private Map<String, List<String>> filterByMoreThanOrEqualsTwoRecords(Map<String, List<String>> organizedresult) {
        return organizedresult.entrySet().stream()
            .filter(record->record.getValue().size() >= 2)
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }

    private List<RankingDTO> mountRanking(Map<String, List<String>> organizedresult) {
        return organizedresult.entrySet().stream()
            .map(record->buildResultRankingDTO(record.getKey(), record.getValue()))
            .collect(Collectors.toList());
    }

    private RankingDTO buildResultRankingDTO(String name, List<String> years){
    ...
    private int subtractValue(int num1, int num2) {return Math.abs(num1 - num2);}
    private int minValue(int y1, int y2) {return y1 < y2 ? y1: y2;}
    private int maxValue(int y1, int y2) {return y1 > y2 ? y1: y2;}
